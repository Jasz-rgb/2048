...
# Game
boardSize = 4
score = 0  # Define score globally
ai = False  # Initialize ai variable

def drawBoard(screen, board, score, moves):
    """
    Draws the game board and tiles on the screen.
    """
    screen.fill(black)  # Filling the screen with white color
    ...
    screen.blit(fontImage, (1, playRegion[1]+1))  # Drawign the score on the screen
    fontImage = scoreFont.render("Moves: " + str(moves), 1, white)  # Rendering the moves
    screen.blit(fontImage, (100, playRegion[1]+1))  # Drawign the moves on the screen
...
def handleInput(event, board, score, moves):
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_w:
            ...
            score = 0  # Reseting score on restart
            moves = 0  # Reseting moves on restart
            invalid_moves = set()
...
def gameLoop():
    ...
    mp.freeze_support()
    mp.set_start_method('spawn', force=True)  # Start method for multiprocessing    
    pool = mp.Pool(processes = 4)  # Initializing multiprocessing pool
    pygame.init()
    screen = pygame.display.set_mode(size)
    pygame.display.set_caption("2048 Game")
    tileFont = pygame.font.SysFont("", 72)
    scoreFont = pygame.font.SysFont("", 24)
    clock = pygame.time.Clock()  # Initializing clock here
    board = [[0]*boardSize for _ in range(boardSize)]
    board = generate_initial_tiles(board)
    score = 0
    moves = 0
    ...
    while True:
        ...
        # Using Expectiminimax to make moves automatically
        if not is_game_over(board):
            nextBestMove = getNextBestMoveExpectiminimax(board)
            if has_won(board):
        ...
        drawBoard(screen, board, score, moves)
        pygame.display.flip()
        clock.tick(FPS)  # Use clock to control frame rate
